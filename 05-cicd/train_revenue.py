...
def load_and_prepare(cfg):
    ...
    rc = cfg["revenue_model"]
    feature_cols = (
        rc["features"]["categorical_high_cardinality"]
        + rc["features"]["categorical_ohe"]
        + rc["features"]["ordinal"]
        + rc["features"]["numeric"]
    )
    target = rc["features"]["target"]
    ...
    logger.info(f"Revenue stats — mean: ${y.mean():,.0f} | median: ${y.median():,.0f} | max: ${y.max():,.0f}")
    return X, y
...
def main(config_path="config.yaml"):
    ...
    logger.info(f"Revenue pipeline saved to {cfg['revenue_model']['artifact_path']}")
